...
def t_loadcase(quiet=False):
    ...
    tmp1[2].Pc1[1:3] = zeros(2)
    tmp1[2].Pc2[1:3] = zeros(2)
    tmp1[2].Qc1min[1:3] = zeros(2)
    tmp1[2].Qc1max[1:3] = zeros(2)
    tmp1[2].Qc2min[1:3] = zeros(2)
    tmp1[2].Qc2max[1:3] = zeros(2)
    tmp1[3].ang_max[0] = 360
    tmp1[3].ang_min[8] = -360
    ...
    t_is(ppc2.areas,    areas,      12, [t, 'areas'])
    t_is(ppc2.gencost,  gencost,    12, [t, 'gencost'])
    t_ok(strcmp(ppc2.version, '2'), [t, 'version'])
    ...
    ## cleanup
    remove(matfile + '.mat')
    remove(pfmatfile + '.mat')
    remove(matfilev2 + '.mat')
    remove(pfmatfilev2 + '.mat')
